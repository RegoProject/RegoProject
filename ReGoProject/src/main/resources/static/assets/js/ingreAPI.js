...
$(document).ready(function() {
	  ...
	  $('#ingreResponeList').on('click', 'li .delete-button2', function() {
...
function appendResponseToList(response) {
	  ...
	  resultArray.forEach(function (item, index) {
	    ...
	    saveButton.className = "h-12 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-full sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple save-button2" 
	    li.appendChild(saveButton);

	    var deleteButton = document.createElement("button");
	    deleteButton.textContent = "삭제";
	    deleteButton.className = "h-12 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-yj border border-transparent rounded-lg sm:w-full sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple delete-button2"; // CSS 클래스 추가
	  ...
	  addIngredientButton.addEventListener("click", function () {
		    ...
		    if (liElements.length < maxItems) {
		      ...
		      newSaveButton.className = "h-12 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-full sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple save-button2";
		      newLi.appendChild(newSaveButton);

		      var newDeleteButton = document.createElement("button");
		      newDeleteButton.textContent = "삭제";
		      newDeleteButton.className = "h-12 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-yj border border-transparent rounded-lg sm:w-full sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple delete-button2";
...
$(document).ready(function() {
  ...
  $('#ingreResponeList').on('click', 'li .save-button2', function() {
    ...
    data: inputData,
     success: function(response) {
        ...
        if (response.failedIngredients.length === 0) {
            ...
            message += '<br>다음 재료는 존재하지 않습니다: ' + response.failedIngredients.join(', ');
        }
